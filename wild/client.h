...
#pragma region TCP
		...
		//if async_write was called and is still not done. this whole write queue stuff is b/c
		//asio doesn't let you call async_write while another async_write is in progress.
...
#pragma region PacketParser
		//packet parsing works like this: first read_state is set to read_length_varint.
		//when the length varint is completed reading, then we wait for all of the data
		//to be sent to the client, before reading the whole packet at once.
		//after we are done reading a complete packet, we call reset_reader()
