...
w_clientbound_packet::w_clientbound_packet(int id) : id(id)
{
}
void w_clientbound_packet::write_bool(bool b)
{
	this->data.push_back(b);
}
void w_clientbound_packet::write_i8(int8_t i8)
{
	this->data.push_back(i8);
}
void w_clientbound_packet::write_u8(uint8_t u8)
{
	this->data.push_back(u8);
}
void w_clientbound_packet::write_i16(int16_t i16)
{
	this->data.push_back((i16 & 0xff00) >> 8);
	this->data.push_back(i16 & 0xff);
}
void w_clientbound_packet::write_u16(uint16_t u16)
{
	this->data.push_back((u16 & 0xff00) >> 8);
	this->data.push_back(u16 & 0xff);
}
void w_clientbound_packet::write_i32(int32_t i32)
{
...
void w_clientbound_packet::write_i64(int64_t i64)
{
...
void w_clientbound_packet::write_float(float f)
{
	int32_t temp = std::bit_cast<int32_t>(f);
	this->write_i32(temp);
}
void w_clientbound_packet::write_double(double d)
{
	int64_t temp = std::bit_cast<int64_t>(d);
	this->write_i64(temp);
}
void w_clientbound_packet::write_string(std::string s)
{
...
void w_clientbound_packet::write_varint(int32_t varint)
{
